...
var ddladdproject = document.querySelector('#filter_site_project');
var prevEladdproject;
var prevEladdproject2;
var prevEladdproject3;

function changeDDL() {
    if (prevEladdproject) {
        prevEladdproject.style.display = "none";
    }
    if (prevEladdproject2) {
        prevEladdproject2.style.display = "none";
    }
    if (prevEladdproject3) {
        prevEladdproject3.style.display = "none";
    }
    var id = ddladdproject.value;
    if (id == 'fixed') {
        var fixed_hourly = document.querySelector('#fixed_hourly');
        fixed_hourly.style.display = "block";

        var project_hourly = document.querySelector('#project_hourly');
        project_hourly.style.display = "none";

        var project_btn = document.querySelector('#project_btn');
        project_btn.style.display = "block";

        prevEladdproject = fixed_hourly;
        prevEladdproject2 = project_hourly;
        prevEladdproject3 = project_btn;
    } else if (id == 'hourly') {
        var project_hourly = document.querySelector('#project_hourly');
        project_hourly.style.display = "block";
        ...
        var fixed_hourly = document.querySelector('#fixed_hourly');
        fixed_hourly.style.display = "none";
        ...
        prevEladdproject = fixed_hourly;
        prevEladdproject2 = project_hourly;
        prevEladdproject3 = project_hourly;
    }
}


// true and false
var ddl = document.querySelector('#filter_hireme_site_project');
var prevEl;

function hiremechangeDDL() {
    if (prevEl) {
...
function populateStates2(countryElementId, stateElementId) {
    var selectedCountryIndex = document.getElementById(countryElementId).selectedIndex;
    var stateId = document.getElementById('operation_city_id').value;
    var state_arr = s_a[selectedCountryIndex].split("|");
    var html;
    for (var i = 0; i < state_arr.length; i++) {
        console.log(state_arr[i]);
        if(stateId == state_arr[i]){
            html += '<option value="'+state_arr[i]+'" selected>'+state_arr[i]+'</option>';
        }else{
            html += '<option value="'+state_arr[i]+'">'+state_arr[i]+'</option>';
        }
    }
    console.log(html);
    $('#state2').append(html);
...
function populateCountries2(countryElementId, stateElementId) {
    ...
    if(document.getElementById('operation_country_id') != null){
        var countryId = document.getElementById('operation_country_id').value;
        var html = '<option value="-1">Select Country</option>';
        for (var i = 0; i < country_arr.length; i++) {
            if(countryId == country_arr[i]){
                html += '<option value="'+country_arr[i]+'" selected>'+country_arr[i]+'</option>';
            }else{
                html += '<option value="'+country_arr[i]+'">'+country_arr[i]+'</option>';
            }
        }
        $('#country2').append(html);
        $('#country2').val(countryId).change();
        $('#country2').val(countryId).trigger('change');
...
populateCountries2("country2", "state2");
